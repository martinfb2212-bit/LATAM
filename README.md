...
3. **Customer Intelligence** — Concentration risk (Pareto 80/20), top growing & declining customers, weekly run-rate vs prior year, new customer tracking with weekly trajectory, plus a **Product Trend Analysis** master table showing FOB per `customer × country × crop × year` from 2022 onward. Each row computes a baseline (average of prior years) and the current-year deviation against it; combinations more than ±25% off baseline are flagged so strong drops or gains are surfaced early. Local filters: crop multi-select, flag selector (Strong drops / Strong gains / Out of trend / On trend / New / Lost / All), sort by largest deviation, current FOB, or customer name. Results respect the global Years / Customers / Countries / YTD-vs-Full-Year filters and are exportable to Excel.
...
### v2.3 — Product Trend Analysis (Customer × Crop × Year)
- New section inside Customer Intelligence: master table with FOB per customer / country / crop / year going back to 2022, with per-row baseline (average of prior years), current-year deviation, and ±25% flag (Strong drop / Strong gain / On trend / New / Lost).
- Local filters: crops multi-select, flag selector, sort selector. Inherits the global Years / Customers / Countries / YTD-vs-Full-Year filters from Commercial Intelligence.
- Summary counters at the top (combinations, drops, gains, lost, new, on trend) and Excel export of the filtered table.
- Trend logic covered by `test_trend.py` (10 unit checks).
