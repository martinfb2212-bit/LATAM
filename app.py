...
def check_credentials(username, password):
    ...
    return u in users and users[u].get("password","")==password.strip()
...
def render_admin():
    ...
    if st.button("Save User",key="adm_save"):
        ...
        if not u: st.warning("Username cannot be empty.")
            ...
            users[u]={"password":np_ if np_ else users.get(u,{}).get("password",""),"display":nd.strip() or u.title(),"role":nr}
    ...
    if st.button("Update Password",key="cp_btn"):
        ...
        if not check_credentials(me,cur_pw): st.error("Current password incorrect.")
            ...
            users[me]["password"]=new_pw; _save_users(users); st.success("Password updated.")
...
def render_commercial(df):
    ...
    with ci_tabs[1]:
        ...
        if tgt_key not in st.session_state:
            if cur_year==2026:
                st.session_state[tgt_key]={c:DEFAULT_TARGETS_2026.get(c,0.0) for c in all_tgt_countries}
            else:
        ...
        with st.form("target_form"):
            ...
            if st.form_submit_button("Save Targets  →"):
                st.session_state[tgt_key]=new_targets; st.success("Targets saved.")
