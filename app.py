...
def hero(title: str, subtitle: str, color: str = "#9C4A52"):
    st.markdown(f"""
...
def section_card(label: str, accent: str = "#9C4A52"):
    st.markdown(f"""
...
def alert_card(msg: str, accent: str = "#8A9E85"):
    st.markdown(f"""
    <div style="background:#FFFFFF;border:1px solid #EDE8E0;border-left:3px solid {accent};
...
def metrics_row(orders: int, qty, fob):
...
def load_and_validate(uploaded) -> tuple[pd.DataFrame | None, str]:
    try:
...
def filter_week(df: pd.DataFrame, year: int, week: int) -> pd.DataFrame:
...
def apply_filters(df: pd.DataFrame, origins, customers) -> pd.DataFrame:
    if origins:
...
# ── ISO week arithmetic ───────────────────────────────────────────────────────
def add_weeks(year: int, week: int, delta: int):
    d = date.fromisocalendar(year, week, 1)
    import datetime as dt
    d2 = d + dt.timedelta(weeks=delta)
    iso = d2.isocalendar()
    return iso[0], iso[1]
...
with st.sidebar:
    ...
    if "df" in st.session_state and st.session_state.df is not None:
        ...
        <div style="background:#3a3330;border-radius:6px;padding:12px 14px;margin-top:8px;
          ...
          <span style="color:#8A9E85;">Records:</span> {len(df_meta):,}
        </div>""", unsafe_allow_html=True)
        ...
        origins   = st.multiselect("Origin", sorted(df_meta["supply_source_name"].dropna().unique()), placeholder="All origins")
        customers = st.multiselect("Customer", sorted(df_meta["customer_name"].dropna().unique()), placeholder="All customers")
...
if "df" not in st.session_state or st.session_state.df is None:
    ...
    <div style="background:#FFFFFF;border:1px solid #EDE8E0;border-radius:8px;
      ...
      <div style="font-family:'Jost',sans-serif;font-size:.84rem;color:#2C2825;
        ...
        The dashboard will refresh automatically each time you upload a new file.
      </div>
...
filt_o = st.session_state.get("origins",   [])
filt_c = st.session_state.get("customers", [])
df     = apply_filters(df, filt_o, filt_c)
